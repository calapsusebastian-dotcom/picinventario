    ...
    <div class="wrap">
        ...
        <div class="topbar">
            <div class="brand">
                ...
                <div>
                    <h1>Productos · Bodega PIC</h1>
                    <p class="subtitle">Catálogo de calidades/productos que alimenta las listas desplegables del formulario</p>
                </div>
            </div>
                    ...
                    <tbody>
                        @forelse ($productos as $p)
                            <tr class="data-row" wire:click="openEdit({{ $p->id }})">
                                <td>{{ $p->nombre }}</td>
                                <td>
                                    <button type="button" class="icon-btn" title="Editar" wire:click.stop="openEdit({{ $p->id }})">
                                    ...
                                    <button type="button" class="icon-btn danger" title="Eliminar" wire:click.stop="confirmDelete({{ $p->id }})">
                            ...
                            @if ($confirmDeleteId === $p->id)
                                <tr class="confirm-row">
                                    <td colspan="2">
                                        ...
                                        <button type="button" class="btn-del-confirm" wire:click.stop="delete({{ $p->id }})">Eliminar</button>
                                        <button type="button" class="btn-del-cancel" wire:click.stop="cancelDelete">Cancelar</button>
                                    </td>
                                </tr>
                            @endif
                        @empty
                            <tr class="empty-row"><td colspan="2">No hay productos que coincidan con la búsqueda.</td></tr>
                        @endforelse
                    </tbody>
                </table>
            </div>
        ...
        <div class="drawer">
            ...
            <form wire:submit.prevent="save">
                <div class="tab-panel active">
                    <div class="field">
                        <label>Nombre</label>
                        <input wire:model="nombre" placeholder="Ej. Rainforest">
