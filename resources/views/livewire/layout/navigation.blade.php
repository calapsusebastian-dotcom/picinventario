...
<nav x-data="{ open: false }" class="bg-white dark:bg-gray-800 border-b border-gray-100 dark:border-gray-700">
            ...
            <div class="flex">
                ...
                <div class="hidden space-x-8 sm:-my-px sm:ms-10 sm:flex">
                    ...
                    @foreach ($this->stageLinks() as $link)
                        <x-nav-link :href="route('inventario.stage', $link['stage'])" :active="request()->routeIs('inventario.stage') && request()->route('stage') === $link['stage']" wire:navigate>
                            {{ $link['label'] }}
                        </x-nav-link>
                    @endforeach
                    @if ($this->canAccessModule('trilla'))
                        <x-nav-link :href="route('trilla')" :active="request()->routeIs('trilla')" wire:navigate>
                            {{ __('Trilla') }}
                        </x-nav-link>
                    @endif
                    ...
                    @if (auth()->user()->isAdmin())
                        ...
                        <x-nav-link :href="route('clientes')" :active="request()->routeIs('clientes')" wire:navigate>
                            {{ __('Clientes') }}
                        </x-nav-link>
                    @endif
                </div>
    ...
    <div :class="{'block': open, 'hidden': ! open}" class="hidden sm:hidden">
        <div class="pt-2 pb-3 space-y-1">
            ...
            @endif
            @foreach ($this->stageLinks() as $link)
                <x-responsive-nav-link :href="route('inventario.stage', $link['stage'])" :active="request()->routeIs('inventario.stage') && request()->route('stage') === $link['stage']" wire:navigate>
                    {{ $link['label'] }}
                </x-responsive-nav-link>
            @endforeach
            @if ($this->canAccessModule('trilla'))
                <x-responsive-nav-link :href="route('trilla')" :active="request()->routeIs('trilla')" wire:navigate>
                    {{ __('Trilla') }}
                </x-responsive-nav-link>
            @endif
            @if ($this->canAccessModule('despacho'))
